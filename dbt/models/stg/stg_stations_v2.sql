            ...
            case
                ...
                then
                    st_makeline(
                        array_agg(
                            distinct st_point(
                                round(startstationlongitude, 6),
                                round(startstationlatitude, 6)
                            )
                        )
                    )
                else
                    st_makeline(
                        st_point(
                            round(max(startstationlongitude), 6),
                            round(max(startstationlatitude), 6)
                        ),
                        st_point(
                            round(max(startstationlongitude), 6),
                            round(max(startstationlatitude), 6)
...
select
    year,
    row_number() over (
        partition by year order by arrondissement, station_name
    ) as rg_station_yearly,
